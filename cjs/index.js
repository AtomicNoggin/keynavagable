'use strict';
const KeyboardEvent = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(require("keyboardevent-key-polyfill"));
...
const KeyNavagable = {
  getTabNext(element) {
    ...
    if (tabs && tabs[next]) {
      ...
      return (
        self.document && document.getElementById(element.getAttribute(tabnext))
      );
    }
  },
  ...
  getTabPrevious(element) {
    ...
    if (tabs && tabs[prev]) {
      ...
      const attrId = element.getAttribute(tabprev);
      ...
      // otherwise do a reverse lookup for an Element that points to this via flowto
        ...
        return (
          self.document &&
          document.querySelector(`[${tabnext}="${element.id}"]`)
        );
      }
      return null;
    }
...
Object.defineProperty(exports, '__esModule', {value: true}).default = KeyNavagable;
